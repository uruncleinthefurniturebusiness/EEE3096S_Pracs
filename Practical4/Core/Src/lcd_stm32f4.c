...
void lcd_clear_line(void) {
    lcd_putstring("                "); // Assuming a 16x2 LCD, write 16 spaces
}

void lcd_update_line(char* new_string) {
    lcd_command(0x80); // Set cursor to the beginning of the line (first line)
    lcd_clear_line();  // Clear the line
    lcd_command(0x80); // Return cursor to the beginning again
    lcd_putstring(new_string); // Write new content
}
